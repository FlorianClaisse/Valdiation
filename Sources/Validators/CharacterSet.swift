...
//
//
//  Created by Florian Claisse on 23/11/2023.
//
...
extension ValidatorResults {
    ...
    public struct CharacterSet {
        ...
        fileprivate var invalidRange: Swift.Range<String.Index>? {
            self.string.rangeOfCharacter(from: self.characterSet.inverted)
        }
        
        fileprivate var invalidSlice: String? {
            self.invalidRange.flatMap { self.string[$0] }
                .map { .init($0 )}
        }
        
        fileprivate var allowedCharacterString: String {
...
extension ValidatorResults {
    ...
    public struct CollectionCharacterSet {
        ...
        /// On validation failure, the first substring of the input with characters not contained in `characterSet`.
        fileprivate var invalidRanges: [(Int, Swift.Range<String.Index>)?] {
