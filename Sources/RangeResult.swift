...
/// Type used by ``ValidatorResults/Range`` and ``Validator/count(_:)-81fxv`` validators to indicate where a value fell within a range.
public enum RangeResult<T>: Equatable where T: Comparable {
    ...
    internal var isWithinRange: Bool {
        switch self {
    ...
    internal init(min: T?, max: T?, value: T) throws {
...
internal enum RangeResultError: Error {
    case notComparable
}
